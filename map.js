...
function Map(start_pt, deg, width) {
    this.walls = [{
        walls: [],
        bb: []
    }] //making the 0th chunk here itself
    this.wallHeight = 2
    ...
    this.checkCollisionForVehicle = function (vehicle) {
        ...
        chunk_list[0] = m.walls[vehicle.inWhichChunk]
        chunk_list[1] = m.walls[vehicle.inWhichChunk - 1]
        chunk_list[2] = m.walls[vehicle.inWhichChunk + 1]

        var is_new_chunk_set = false
        var i = 0
        for (var chunk of chunk_list) {

            if(chunk != undefined){

                for(var wall of chunk.walls){
                ...
                if(!is_new_chunk_set){
                    if(is_within_polygon(chunk.bb, vehicle.mesh.position)){
                        vehicle.inWhichChunk = m.walls.indexOf(chunk)
                        is_new_chunk_set = true
                    }
                }
            }
